...
# inspired by code developed by Trevor Beaton for Adafruit Industries
# SPDX-License-Identifier: MIT
#
# MatrixWeather_AIO_Receiver
...
def update_display():
    ...
    # Get the local time
    try:
        mp.get_local_time("America/Los_Angeles")
    ...
    mp.set_text(wind, 2)
    mp.set_text(f"{current_condition} Gusts:{wind_gusts} Shop:{shop_temperature}", 3)

    # Display icon based on filename determined earlier
...
old_prog_bar_x = -1  # Initialize progress bar
...
while True:
    ...
    # Update weather every UPDATE_DELAY seconds
    if current_time - last_weather_update > UPDATE_DELAY:
        update_display()
        last_weather_update = current_time

    # Update the progress bar
    prog_bar_x = int(round(32 * (current_time - last_weather_update) / UPDATE_DELAY, 0))
